...
@dag(
    ...
    on_failure_callback=partial(
        ...
        "events",
        "failed",
        message="Google Analytics 4 events haven't been updated due to an error",
    ),
    on_success_callback=partial(
        ...
        "events",
        "success",
        message="Google Analytics 4 events have been updated successfully",
    ),
)
def ga4_loader():
    ga4_loader_days_ago = Variable.get("ga4_loader_days_ago", 7)

    load_intraday = TriggerDagRunOperator(
        task_id=f"load_intraday",
        trigger_dag_id="ga4_loader_intraday",
    ...
    load_yesterday = TriggerDagRunOperator(
        ...
        trigger_dag_id="ga4_loader_daily",
        execution_date=pendulum.today().subtract(days=1),
        wait_for_completion=True,
        reset_dag_run=True,
    )
    ...
    for i in range(2, ga4_loader_days_ago + 1):
...
def get_days_sequence(**kwargs):
    ...
    ga4_loader_from_date = kwargs["dag_run"].conf["from_date"]
    start = pendulum.from_format(ga4_loader_from_date, "YYYY-MM-DD")
    period = pendulum.period(start, pendulum.yesterday())
    return [dt.format("YYYY-MM-DD") for dt in period.range("days")]


@task(task_id="ga4_from_date_loader")
def ga4_from_date_loader(day: str, **kwargs):
...
@dag(
    ...
    tags=["google-analytics", "ga4", "snowflake", "big-query", "gcs"],
    on_failure_callback=partial(
        alert_slack_channel, "ga4", "ga_sessions", "failed"
    ),
)
def ga4_loader_from_date():
    ga4_from_date_loader.expand(day=get_days_sequence())


ga4_loader_dag = ga4_loader()
ga4_loader_from_date_dag = ga4_loader_from_date()
